...
describe('agent trace list', () => {
  ...
  describe('--agent filter', () => {
    it('returns only traces for the matching agent', async () => {
      const result = await AgentTraceList.run(['--agent', 'My_Agent_A']);
      expect(result).to.have.length(2);
      expect(result.every((r: any) => r.agent === 'My_Agent_A')).to.be.true;
    });

    it('uses case-insensitive substring match', async () => {
      const result = await AgentTraceList.run(['--agent', 'agent_a']);
      expect(result).to.have.length(2);
    });

    it('returns empty when no agents match', async () => {
      const result = await AgentTraceList.run(['--agent', 'NonExistent']);
      expect(result).to.deep.equal([]);
    });
  });
  ...
  describe('--session-id filter', () => {
    it('returns only traces for the specified session', async () => {
      const result = await AgentTraceList.run(['--session-id', 'sess-1']);
      expect(result).to.have.length(2);
      expect(result.every((r: any) => r.sessionId === 'sess-1')).to.be.true;
    });

    it('returns empty when session ID does not match', async () => {
      const result = await AgentTraceList.run(['--session-id', 'no-such-session']);
      expect(result).to.deep.equal([]);
    });
  });
  ...
  describe('--since filter', () => {
    ...
    it('rejects an invalid date format', async () => {
      try {
        await AgentTraceList.run(['--since', '43/3']);
    ...
    it('rejects a plain non-ISO string', async () => {
      try {
        await AgentTraceList.run(['--since', 'last-week']);
        expect.fail('Should have thrown');
      } catch (err: unknown) {
        expect((err as Error).message).to.match(/invalid.*since|InvalidDate/i);
      }
  ...
  describe('combined filters', () => {
    ...
    it('applies --session-id and --agent together', async () => {
      const result = await AgentTraceList.run(['--agent', 'My_Agent_A', '--session-id', 'sess-1']);
      expect(result).to.have.length(2);
      expect(result.every((r: any) => r.sessionId === 'sess-1')).to.be.true;
    });
  });
